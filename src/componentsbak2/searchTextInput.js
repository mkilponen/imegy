    ...
    <Downshift
      ...
      >
        {({
          ...
          <Paper {...getMenuProps()} style={{zIndex: 2, backgroundColor: style.paperColor, maxHeight:280, overflowY: 'scroll', scrollbarWidth: 'none'}}>
              {
                props.isOpen
                ? props.items.filter(item => !inputValue || item.value.includes(inputValue))
                    .map((item, index) => (
                      ...
                      >
                        {item.value} |
                        <p style={{color: '#969696', top: 7, position: 'relative', fontSize: 12}}>by {item.owner}</p>
                      </MenuItem>
                    ))
                : null}
            </Paper>
